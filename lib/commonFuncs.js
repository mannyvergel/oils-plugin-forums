...
exports.addActiveUser = function (topicOrCatId, user, callback) {
    ...
    this.findOne({_id: topicOrCatId}, function(err, rec) {
        if (rec) {
            ...
            //insert in top most
            arrActiveUsers.splice(0, 0, {_id: user._id, username: user.username, nickname: user.nickname, avatar: user.avatar});
            ...
            ModelObj.updateOne({_id: topicOrCatId}, {$set: {activeUsers: arrActiveUsers}}, function(err, count) {
                if (err) {
                    console.error(err);
                }
...
exports.incrementViewCount = function (topicOrCatId, callback) {
  ...
  return this.updateOne({_id: topicOrCatId}, { $inc: { viewCount: 1 }, $setOnInsert: setOnInsert}, 
